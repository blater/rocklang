...
Table 3: Return values by data type
...
As you can see from the program in Listing below, the parameters are passed to the function 
via the stack. The state of the stack at the start of the pokevalue call will be:

Address	Parameters used in the listing above
+-----+--------------------------------+
...
```
void pokevalue(int address, int value){
  #asm
    pop DE // return address - move this out of the way... (10 cycles)

    // value will be a 16 bit value a the top of the stack
    pop BC   // we will use the value in C only. (10 cycles)

    // address will be next on the stack (10 cycles)
    pop HL

    // put the byte in C into the address held in HL
    LD (HL), C
    
    // return. 
    //   NOTE that by convention the value in HL is returned.
    //   so in this case we will return value of the address parameter as a side effect
