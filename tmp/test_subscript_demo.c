...
int main(int argc, char **argv) {
  ...
  {
    __internal_dynamic_array_t people =
        __internal_make_array(sizeof(Person), 2);
    struct Person tmp_fred = {
        .id = 1,
        .name = new_string((string){.data = "Frederick", .length = 9})};
    Person fred = allocate_compiler_persistent(sizeof(struct Person));
    *fred = tmp_fred;
    struct Person tmp_charles = {
        .id = 2, .name = new_string((string){.data = "Chuck", .length = 5})};
    Person charles = allocate_compiler_persistent(sizeof(struct Person));
    *charles = tmp_charles;
    Person_set_elem(people, (size_t)(0), fred);
    Person_set_elem(people, (size_t)(1), charles);
    print(Person_get_elem(people, (size_t)(0))->name);
    print(new_string((string){.data = "\n", .length = 1}));
    print(Person_get_elem(people, (size_t)(1))->name);
    print(new_string((string){.data = "\n", .length = 1}));
    Person p = Person_get_elem(people, (size_t)(0));
    Person_set_elem(people, (size_t)(1), p);
    print(Person_get_elem(people, (size_t)(0))->name);
    print(new_string((string){.data = "\n", .length = 1}));
    print(Person_get_elem(people, (size_t)(1))->name);
    print(new_string((string){.data = "\n", .length = 1}));
