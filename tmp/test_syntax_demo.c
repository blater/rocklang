...
__internal_dynamic_array_t Point_make_array(void) {
  return __internal_make_array(sizeof(Point), 0);
}

void Point_push_array(__internal_dynamic_array_t arr, Point elem) {
  __internal_push_array(arr, &elem);
}

Point Point_pop_array(__internal_dynamic_array_t arr) {
  Point *res = __internal_pop_array(arr);
  return *res;
}

Point Point_get_elem(__internal_dynamic_array_t arr, size_t index) {
  Point *res = __internal_get_elem(arr, index);
  if (res == NULL) {
    printf("NULL ELEMENT IN Point_get_elem");
    exit(1);
  }
  return *res;
}

void Point_set_elem(__internal_dynamic_array_t arr, size_t index, Point elem) {
  __internal_set_elem(arr, index, &elem);
}

void Point_insert(__internal_dynamic_array_t arr, size_t index, Point elem) {
  __internal_insert(arr, index, &elem);
}

__internal_dynamic_array_t Item_make_array(void) {
  return __internal_make_array(sizeof(Item), 0);
}

void Item_push_array(__internal_dynamic_array_t arr, Item elem) {
  __internal_push_array(arr, &elem);
}

Item Item_pop_array(__internal_dynamic_array_t arr) {
  Item *res = __internal_pop_array(arr);
  return *res;
}

Item Item_get_elem(__internal_dynamic_array_t arr, size_t index) {
  Item *res = __internal_get_elem(arr, index);
  if (res == NULL) {
    printf("NULL ELEMENT IN Item_get_elem");
    exit(1);
  }
  return *res;
}

void Item_set_elem(__internal_dynamic_array_t arr, size_t index, Item elem) {
  __internal_set_elem(arr, index, &elem);
}

void Item_insert(__internal_dynamic_array_t arr, size_t index, Item elem) {
  __internal_insert(arr, index, &elem);
}
...
int main(int argc, char **argv) {
  ...
  {
    print(new_string((string){.data = "=== Syntax Demo ===\n", .length = 20}));
    int a = 10;
    int b = 20;
    int c = a + b;
    print(new_string((string){.data = "a := ", .length = 5}));
    print_int(a);
    print(new_string((string){.data = "\n", .length = 1}));
    print(new_string((string){.data = "b => ", .length = 5}));
    print_int(b);
    print(new_string((string){.data = "\n", .length = 1}));
    print(new_string((string){.data = "c := ", .length = 5}));
    print_int(c);
    print(new_string((string){.data = "\n", .length = 1}));
    print(
    ...
    print(new_string((string){.data = "\n", .length = 1}));
    print(
        new_string((string){.data = "Loop without => arrow:\n", .length = 23}));
    for (int i = 0; i <= (int)2; i++) {
      print_int(i);
      print(new_string((string){.data = " ", .length = 1}));
    };
    print(new_string((string){.data = "\n", .length = 1}));
    print(new_string((string){.data = "=== End Demo ===\n", .length = 17}));
  }
  kill_compiler_stack();
  return 0;
}
