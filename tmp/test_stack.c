...
__internal_dynamic_array_t Stack_make_array(void) {
  return __internal_make_array(sizeof(Stack), 0);
}

void Stack_push_array(__internal_dynamic_array_t arr, Stack elem) {
  __internal_push_array(arr, &elem);
}

Stack Stack_pop_array(__internal_dynamic_array_t arr) {
  Stack *res = __internal_pop_array(arr);
  return *res;
}

Stack Stack_get_elem(__internal_dynamic_array_t arr, size_t index) {
  Stack *res = __internal_get_elem(arr, index);
  if (res == NULL) {
    printf("NULL ELEMENT IN Stack_get_elem");
    exit(1);
  }
  return *res;
}

void Stack_set_elem(__internal_dynamic_array_t arr, size_t index, Stack elem) {
  __internal_set_elem(arr, index, &elem);
}

void Stack_insert(__internal_dynamic_array_t arr, size_t index, Stack elem) {
...
Stack push(Stack s, int e) {
  struct Stack tmp_res = {.top = e, .next = s};
  Stack res = allocate_compiler_persistent(sizeof(struct Stack));
  *res = tmp_res;
  return res;
}
...
int pop(Stack s) {
  ...
  print(new_string(
      (string){.data = "Cannot pop an empty stack !\n", .length = 28}));
  exit(1);
}
...
int main(int argc, char **argv) {
  ...
  {
    Stack stk = new_stack();
    string st = new_string((string){.data = "hello ", .length = 6});
    print(st);
