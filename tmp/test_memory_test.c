...
int main(int argc, char **argv) {
  ...
  {
    print(new_string((string){.data = "=== Test 1: Simple string concat ===\n",
                              .length = 37}));
    {
      string s1 = new_string((string){.data = "hello", .length = 5});
      string s2 =
          concat(s1, new_string((string){.data = " world", .length = 6}));
      print(s2);
      print(new_string((string){.data = "\n", .length = 1}));
    }
    print(new_string((string){.data = "=== Test 2: Dynamic string arrays ===\n",
                              .length = 38}));
    {
      __internal_dynamic_array_t names =
          __internal_make_array(sizeof(string), 0);
      string_push_array(names,
                        new_string((string){.data = "Alice", .length = 5}));
      string_push_array(names,
                        new_string((string){.data = "Bob", .length = 3}));
      string_push_array(names,
                        new_string((string){.data = "Charlie", .length = 7}));
      print(string_get_elem(names, 0));
      print(new_string((string){.data = ", ", .length = 2}));
    ...
    print(new_string(
        (string){.data = "=== Test 3: String concatenation in loop ===\n",
                 .length = 45}));
    {
      for (int i = 0; i <= (int)2; i++) {
        string s = concat(new_string((string){.data = "Index ", .length = 6}),
                          toString(i));
        print(s);
        print(new_string((string){.data = "\n", .length = 1}));
      };
    }
    print(new_string((string){.data = "=== Test 4: Records with strings ===\n",
                              .length = 37}));
    {
      struct Person tmp_p1 = {
          .name = new_string((string){.data = "Alice", .length = 5}),
          .age = 30};
      Person p1 = allocate_compiler_persistent(sizeof(struct Person));
      *p1 = tmp_p1;
      struct Person tmp_p2 = {
          .name = new_string((string){.data = "Bob", .length = 3}), .age = 25};
      Person p2 = allocate_compiler_persistent(sizeof(struct Person));
      *p2 = tmp_p2;
      print(p1->name);
      print(new_string((string){.data = " is ", .length = 4}));
    ...
    print(new_string(
        (string){.data = "=== Test 5: Records with dynamic string arrays ===\n",
                 .length = 51}));
    {
      struct Team tmp_team = {
          .leader = new_string((string){.data = "Alice", .length = 5}),
      ...
      *team = tmp_team;
      ...
      string_push_array(team->members,
                        new_string((string){.data = "Charlie", .length = 7}));
      print(new_string((string){.data = "Leader: ", .length = 8}));
      print(team->leader);
      print(new_string((string){.data = "\n", .length = 1}));
      print(new_string((string){.data = "Members: ", .length = 9}));
    ...
    print(new_string((string){
        .data = "=== Test 6: Nested scopes with concat ===\n", .length = 42}));
    {
      for (int i = 0; i <= (int)2; i++) {
        string prefix = concat(
            new_string((string){.data = "Item ", .length = 5}), toString(i));
        print(prefix);
        print(new_string((string){.data = "\n", .length = 1}));
      };
    }
    print(new_string(
        (string){.data = "=== Test 7: Dynamic array append and reassign ===\n",
                 .length = 50}));
    {
      __internal_dynamic_array_t items =
          __internal_make_array(sizeof(string), 0);
      string_push_array(items,
                        new_string((string){.data = "first", .length = 5}));
      print(new_string((string){.data = "Initial: ", .length = 9}));
      print(string_get_elem(items, 0));
      print(new_string((string){.data = "\n", .length = 1}));
      string_set_elem(items, (size_t)(0),
                      new_string((string){.data = "second", .length = 6}));
      print(new_string((string){.data = "After reassign: ", .length = 16}));
      print(string_get_elem(items, 0));
      print(new_string((string){.data = "\n", .length = 1}));
    }
    print(new_string((string){
        .data = "=== Test 8: String concatenation chain ===\n", .length = 43}));
    {
      string result = concat(new_string((string){.data = "Hello", .length = 5}),
                             new_string((string){.data = " ", .length = 1}));
      result =
          concat(result, new_string((string){.data = "World", .length = 5}));
      result = concat(result, new_string((string){.data = "!", .length = 1}));
      print(result);
      print(new_string((string){.data = "\n", .length = 1}));
    }
    print(new_string((string){
        .data = "=== Test 9: Deep nesting with concat ===\n", .length = 41}));
    {
      for (int i = 0; i <= (int)1; i++) {
        for (int j = 0; j <= (int)1; j++) {
          string msg = concat(new_string((string){.data = "i=", .length = 2}),
                              toString(i));
          msg = concat(msg, new_string((string){.data = ", j=", .length = 4}));
    ...
    print(new_string((string){
        .data = "=== Test 10: Many iterations (loop allocation reuse) ===\n",
        .length = 57}));
    {
      for (int i = 0; i <= (int)9; i++) {
        string s = concat(new_string((string){.data = "Count: ", .length = 7}),
                          toString(i));
        print(s);
        print(new_string((string){.data = "\n", .length = 1}));
      };
    ...
    print(new_string(
        (string){.data = "=== All memory tests complete ===\n", .length = 34}));
  }
  kill_compiler_stack();
  return 0;
}
