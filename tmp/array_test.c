...
void test() {
  __internal_dynamic_array_t arr = __internal_make_array(sizeof(int), 0);
  for (int i = 0; i <= (int)10; i++) {
    int_push_array(arr, i);
  };
  for (int i = 0; i <= (int)length(arr) - 1; i++) {
    int n = int_get_elem(arr, i);
    print_int(n);
...
array_wrapper func() {
  ...
  *a = tmp_a;
  ...
  string_push_array(names, new_string(__rock_make_string("First\n", 6)));
  int_push_array(a->Items, 2);
  string_push_array(names, new_string(__rock_make_string("Second\n", 7)));
  int_push_array(a->Items, 3);
  string_push_array(names, new_string(__rock_make_string("Third\n", 6)));
  return a;
}
...
int main(int argc, char **argv) {
  ...
  {
    test();
    __internal_dynamic_array_t pople = __internal_make_array(sizeof(int), 2);
    int_set_elem(pople, (size_t)(0), 99);
    print_int(int_get_elem(pople, (size_t)(0)));
    __internal_dynamic_array_t fixed = __internal_make_array(sizeof(string), 3);
    string_set_elem(fixed, (size_t)(0),
                    new_string(__rock_make_string("hello", 5)));
    string_set_elem(fixed, (size_t)(1),
                    new_string(__rock_make_string("world", 5)));
    print(new_string(__rock_make_string("\n-----\n", 7)));
    print(string_get_elem(fixed, (size_t)(0)));
    print(new_string(__rock_make_string("\n", 1)));
    print(string_get_elem(fixed, (size_t)(1)));
    print(new_string(__rock_make_string("\n", 1)));
    print(concat(string_get_elem(fixed, (size_t)(0)),
                 string_get_elem(fixed, (size_t)(1))));
    print(new_string(__rock_make_string("\n-----\n", 7)));
    __internal_dynamic_array_t strings =
        __internal_make_array(sizeof(string), 0);
    string_push_array(strings, new_string(__rock_make_string("Hey !\n", 6)));
    string_push_array(strings,
                      new_string(__rock_make_string("How are you ?\n", 14)));
    string_push_array(strings, new_string(__rock_make_string("What ???\n", 9)));
    for (int i = 0; i <= (int)length(strings) - 1; i++) {
      string s = string_get_elem(strings, i);
      print(s);
    };
