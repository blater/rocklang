...
{printf(new_string(__rock_make_string("Testing peek/poke on ZXN\n", 25)));
word addr1 = to_word(28672);
byte val1 = to_byte(42);
poke(addr1, val1);
byte result1 = peek(addr1);
if (result1 == val1)
printf(new_string(__rock_make_string("Test 1: PASS (42 stored/retrieved)\n", 35)));
else
printf(new_string(__rock_make_string("Test 1: FAIL\n", 13)));
word addr2 = to_word(28673);
byte val2 = to_byte(100);
poke(addr2, val2);
byte result2 = peek(addr2);
if (result2 == val2)
printf(new_string(__rock_make_string("Test 2: PASS (100 stored/retrieved)\n", 36)));
else
printf(new_string(__rock_make_string("Test 2: FAIL\n", 13)));
printf(new_string(__rock_make_string("peek/poke test complete\n", 24)));
}kill_compiler_stack();
return 0;
}
