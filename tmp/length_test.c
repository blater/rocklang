...
int main(int argc, char **argv) {
  ...
  {
    print(new_string(__rock_make_string("=== String length tests ===\n", 28)));
    string s1 = new_string(__rock_make_string("Hello", 5));
    print(new_string(__rock_make_string("String: ", 8)));
    print(s1);
    print(new_string(__rock_make_string(" Length: ", 9)));
    print_int(length(s1));
    print(new_string(__rock_make_string("\n", 1)));
    string s2 = new_string(__rock_make_string("Rock", 4));
    print(new_string(__rock_make_string("String: ", 8)));
    print(s2);
    print(new_string(__rock_make_string(" Length: ", 9)));
    print_int(length(s2));
    print(new_string(__rock_make_string("\n", 1)));
    string s3 = new_string(__rock_make_string("", 0));
    print(new_string(__rock_make_string("Empty string length: ", 21)));
    print_int(length(s3));
    print(new_string(__rock_make_string("\n", 1)));
    print(new_string(__rock_make_string("\n=== Array length tests ===\n", 28)));
    __internal_dynamic_array_t arr = __internal_make_array(sizeof(int), 0);
    print(new_string(__rock_make_string("Empty array length: ", 20)));
    print_int(length(arr));
    print(new_string(__rock_make_string("\n", 1)));
    int_push_array(arr, 10);
    int_push_array(arr, 20);
    int_push_array(arr, 30);
    print(new_string(__rock_make_string("Array [10,20,30] length: ", 25)));
    print_int(length(arr));
    print(new_string(__rock_make_string("\n", 1)));
    print(new_string(__rock_make_string("\n=== String array tests ===\n", 28)));
    __internal_dynamic_array_t strs = __internal_make_array(sizeof(string), 0);
    string_push_array(strs, new_string(__rock_make_string("one", 3)));
    string_push_array(strs, new_string(__rock_make_string("two", 3)));
    string_push_array(strs, new_string(__rock_make_string("three", 5)));
    print(new_string(
        __rock_make_string("String array [one,two,three] length: ", 37)));
