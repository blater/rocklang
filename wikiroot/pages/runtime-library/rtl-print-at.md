...
Third RTL component under the [[rtl-overview]] conventions. Draws a Rock string at a character cell `(x, y)` on the ZX upper screen by calling ROM `RST 10h` directly, preceded by the `AT` control sequence. The ZXN implementation is a direct port of pasta80's approach (see [[pasta80-target-platforms]] §ZX Spectrum 48K).
...
On the host target, positioned terminal rendering is only active after
`graphics on;`. Without it, `print(x, y, text)` uses the plain-text fallback.
On ZXN, `graphics on;` / `graphics off;` are no-ops because the hardware-backed
...
`print_at.c`'s host branch is a flat capability check — all lifecycle (`isatty` probe, `tb_init`, `atexit` teardown) lives in [[rtl-host-caps]] (`src/lib/host_caps.c`) and is triggered by the Rock statement `graphics on;`. `print_at` only reads `host_caps.print_at`:
...
2. Otherwise (graphics not requested, no tty, or `tb_init` failed) fall back to the plain-text `@(x,y) text\n` form. The test harness keeps working because `run_tests.sh` pipes stdout — the capability stays off.
...
No separate `.asm` file and no exported asm symbols. Inline asm inside the C shim is sufficient here; this is a stronger case than [[rtl-border]] because we need to call a ROM vector, which inline asm handles fine.
...
- `rock test/print_at_test.rkr && ./test/print_at_test.exe` in a real terminal — without `graphics on;`, the host branch uses the plain-text fallback. A manual graphics preview should call `graphics on;` before positioned output and `graphics off;` before exit.
...
- [[rtl-overview]] — conventions and the SDCC underscore-prefix gotcha
- [[rtl-keyboard]], [[rtl-border]] — prior components
- [[pasta80-target-platforms]] — pasta80's ZX48/Next ROM approach (the model)
