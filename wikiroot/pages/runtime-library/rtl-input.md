...
Second keyboard-related component, serving a **completely different need** from [[rtl-keyboard]]. This one hands back a proper ASCII byte decoded by the ROM (Shift, Symbol Shift, caps lock, auto-repeat, the lot); the other exposes raw matrix state for detecting multiple simultaneous key-holds.

**Use this component for:** menus, text entry, "press a key to start", typing games, anything where you want a character.

**Use [[rtl-keyboard]] instead for:** action games where the player holds Z+X+SPACE at the same time, joystick-style controls, per-frame polling.
...
This is exactly the idiom ZX BASIC's `INKEY$` uses and exactly what [[pasta80-rtl-architecture]] does in `zxrom.asm`. There is no Z80 code we own here — the ROM does the hard work.
...
For real action games you want to see `left`, `right`, `fire`, and `jump` all held simultaneously, with no auto-repeat and no modifier semantics. That's what [[rtl-keyboard]]'s `scan_keyboard()` + `key_pressed(KEY_X)` gives you.
...
| Fire + move at the same time | [[rtl-keyboard]] `scan_keyboard` + `key_pressed` | simultaneous keys |
| Action games with jump+shoot+run | [[rtl-keyboard]] | ROM route can't do it |
...
- [[rtl-keyboard]] — the matrix scanner, for simultaneous key-hold detection
- [[rtl-overview]] — conventions
- [[rtl-print-at]] — also calls ROM via `RST 10h`; same "depends on ROM paged in" caveat
- [[pasta80-target-platforms]] — pasta80's `zx_readkey` / `zx_testkey`, the idiom we adopted
