...
**Colour constants** (standard Spectrum palette — `#define` in the public header per [[rtl-overview]] rule 5):
...
- **Direct attribute-memory writes** to `$5800-$5AFF` — part of the raster TODO on [[rtl-print-at]].
...
- [[rtl-print-at]] — host branch gets the `attr_fg`/`attr_bg` reads
- [[rtl-overview]] — rule 1 gains the public-header clarification
- [[rtl-host-caps]] — gains the `ink` capability flag
- [[rtl-border]] — adjacent colour component (border is ULA port, unrelated to character-cell attributes)
