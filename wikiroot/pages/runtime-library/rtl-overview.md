...
8. **Byte-oriented APIs.** Sizes, offsets, and counts in bytes. Avoid record/block abstractions until we meet a use case that needs them — the pasta80 `FileSeek` leak (see [[pasta80-lessons-for-rock]] §8) is the cautionary tale.
...
11. **Lifecycle lives in [[rtl-host-caps]], not in components.** Any RTL component whose host implementation needs to open a terminal, probe `isatty`, call `tb_init`, install an `atexit` handler, or otherwise differ between ZXN (always-faithful) and host (probe-and-maybe-fall-back) MUST add a flag to `rock_host_caps` and read it on each call. Components are forbidden from calling lifecycle primitives directly; `rock_rtl_init()` (called once at startup by the generator-emitted `main()`) is the single place those run.
...
If a component needs a **host-only third-party dependency** (e.g. termbox2), add the single-TU implementation file under `src/lib/host/` and append its path to `RTL_HOST_SRCS` in `rock`. The gcc branch picks it up with any needed `-I` path; the SDCC branch ignores it. See [[rtl-print-at]] for the termbox2 example.
...
- [[rtl-keyboard]] — ZX Spectrum 8×5 matrix scanner, first component, reference implementation for this page.
- [[rtl-host-caps]] — centralised host capability + lifecycle layer (rule 11).

## See Also

- [[pasta80-lessons-for-rock]] — lessons this RTL strategy is built on (partially applied — see this page for the Rock-specific realisation)
- [[generator-overview]] — where `register_builtin` and include emission live
