...
algorithm. Sibling to [[rtl-plot]] and [[rtl-draw]] — same framebuffer,
...
- Honours the sticky global **draw mode** from [[rtl-plot]]:
...
through [[rtl-plot]]'s `plot_nopresent` + `plot_flush` API. On ZXN
...
ships ([[rtl-overview]] roadmap) it will likely expose
...
- [[rtl-plot]] — pixel primitive; `circle` delegates to `plot_nopresent`
- [[rtl-draw]] — line primitive; shares the same draw-mode state
- [[rtl-polyline]] — connected segments; same shape-wrapping idea
