...
`fill_rect` — implemented as a thin wrapper over [[rtl-draw]]'s
...
span per y). It's ~40 lines of its own. Ships under [[rtl-circle]]'s
...
- **Gradient / pattern fills.** No colour control at the pixel level
  on classic ULA (attributes are cell-level via [[rtl-ink-paper]]).
- **Clipping rectangle / state.** Callers clip at the coordinates
  they pass.

## See Also

- [[rtl-draw]] — horizontal fast path that fill_rect delegates to
- [[rtl-plot]] — pixel primitive + draw mode state
- [[rtl-circle]] — outline circle; fill sibling deferred
