...
screen. Sibling to [[rtl-plot]] — same framebuffer, same host shadow,
...
- Origin `(0, 0)` is **top-left**, matching [[rtl-plot]].
- Line drawing respects the global **draw mode** (see [[rtl-plot]]):
...
`plot_flush()` once at the end. Rendering reuses [[rtl-plot]]'s 256×192
...
(declared in [[rtl-plot]]). Two modes ship:
...
- [[rtl-plot]] — pixel primitive; `draw` shares its shadow/flush model
- [[targets/zxn/zxn-ula]] — ULA memory layout, `PIXELAD` / `SETAE` / `PIXELDN`
- [[rtl-overview]] — RTL component conventions
- [[rtl-host-caps]] — host capability flag reused from `plot`
