...
- [x] Generate `__retain_T` and `__release_T` for every used record, union, and module type. — All three aggregate kinds use the same header layout, so a single pair (`__handle_retain` / `__handle_release`) serves them all instead of per-type emission. Generator emits `__handle_release` at scope cleanup for every TRACK_HANDLE slot, releases-on-overwrite at identifier and field reassignment, and retains on borrower init/reassignment. Producer RHS (funcalls returning aggregates) flows through `__return_handle` and so already brings rc=1 to the caller.
...
- [~] Emit retain/release on assignment, overwrite, slot write, field write, scope exit, and discarded producer temporaries. — Aggregates (records/unions/modules) fully covered: `generate_assignement` releases the old handle before identifier or field overwrite and retains the new one on borrower RHS; `generate_vardef` retains on borrower init. `__string_release` is paired with every `__free_string` emission for the string analogue. String borrower-retain at the new producer-allocating sites still rides on Phase H, when `backing` becomes the canonical lifetime marker. Array slot writes are blocked on Phase D-array work.
