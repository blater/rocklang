...
- No colour argument. Pixels inherit the attribute cell they land in,
  whose ink/paper comes from the current [[rtl/rtl-ink-paper]] state.
- `plot` honours the sticky global **draw mode** — `DRAW_MODE_OR`
  ...
  `set_draw_mode(to_byte(0|1))`. Also respected by [[rtl/rtl-draw]].
...
the whole point of the raster migration tracked on [[rtl/rtl-print-at]].
...
**Colour.** Pass `attr_fg()`/`attr_bg()` from [[rtl/rtl-ink-paper]], same
...
- [[rtl/rtl-print-at]] — shares the future raster path
- [[rtl/rtl-ink-paper]] — attribute source for host-side colouring
- [[rtl/rtl-host-caps]] — gains the `plot` capability flag
- [[rtl/rtl-overview]] — component conventions
