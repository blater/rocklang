...
mode, same host shadow as [[rtl/rtl-plot]] / [[rtl/rtl-draw]].
...
- Colour comes from whichever attribute cell each pixel lands in —
  set the ink via [[rtl/rtl-ink-paper]] before drawing. The framebuffer
...
- Both routines honour the current **draw mode** from [[rtl/rtl-plot]]:
...
edge). See [[rtl/rtl-fill]]'s non-goals for the same reasoning applied
...
- [[rtl/rtl-draw]] — line primitive used for each edge and each fill span
- [[rtl/rtl-fill]] — same "walk y, call draw per row" pattern for rects
- [[rtl/rtl-plot]] — pixel primitive underlying everything
- [[rtl/rtl-ink-paper]] — attribute cell colour control
